...
import ldm.train_state
import ldm.utils as utils
import ldm.dataset as dataset
...
class Experiment(ABC):
  ...
  def __init__(self, config: ml_collections.ConfigDict):
    ...
    self.state = ldm.train_state.TrainState.create(
        apply_fn=self.model.apply,
        variables=FrozenDict(params),
    ...
    if ckpt_restore_dir != 'None':
      if 'ckpt-' in ckpt_restore_dir.split('/')[-1]:
        ckpt_restore = checkpoint.Checkpoint(
          os.path.dirname(ckpt_restore_dir))
        state_restore_dict = ckpt_restore.restore_dict(ckpt_restore_dir)
      else:
        ...
        del checkpoint_to_restore
      self.state = restore_partial(self.state, state_restore_dict)
      del state_restore_dict, ckpt_restore
    ...
    if config.name == 'adamw':
      def flattened_traversal(fn):
        def mask(data):
          ...
          # https://github.com/netket/netket/issues/916#issuecomment-915846418
          return flax.core.freeze(
            flax.traverse_util.unflatten_dict({k: fn(k, v) for k, v in flat.items()}))
        return mask
      optimizer_others = optax.adamw(
          learning_rate=lr,
          mask=decay_mask_fn,
          **config.args,
      )
      optimizer_gamma_network = optax.adamw(
          learning_rate=lr,
          mask=decay_mask_fn,
          **config.args,
      )

      def _score_params(path, _unused):
        assert path[0] in {'encoder_model', 'score_model', 'gamma'}
        return path[0] == 'score_model'
      def _not_score_params(path, _unused):
        assert path[0] in {'encoder_model', 'score_model', 'gamma'}
        return path[0] != 'score_model'
      
      optimizer = optax.chain(
        optax.masked(optimizer_others, mask=flattened_traversal(_score_params)),
        optax.masked(optimizer_gamma_network, mask=flattened_traversal(_not_score_params)),
      )
  ...
  def train_and_evaluate(self, workdir: str):
    ...
    ckpt = checkpoint.MultihostCheckpoint(checkpoint_dir, max_to_keep=100)
  ...
  def train_step(self, base_rng, state, batch):
    ...
    (_, metrics), grads = grad_fn(state.params, batch, step=state.step, rng=rng, is_train=True)
  ...
  def eval_step(self, base_rng, params, batch, eval_step=0):
    ...
    _, metrics = self.loss_fn(params, batch, eval_step, rng=rng, is_train=False)
