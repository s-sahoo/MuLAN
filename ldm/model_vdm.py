...
class VDMConfig:
  ...
  forward_n_layer: int = 4
  forward_type: int = 1

  sigma_type: str = 'learnable_scalar'
  sigma_min: float = 0
  ...
  sigma_recons_type: str = 'sigmoid'
  loss_type: str = 'recons'
  reparam_type: str = 'noise'
  nn_input: str = 'gamma'
  condition: str = 'label'
  latent_size: int = 10
  epsilon: float = 0.0
  ...
  monotone_layer: str = 'dense_monotone'
  latent_type: str = 'gumbel'
  z_conditioning: bool = False
  importance_sampling: bool = False
  velocity_from_epsilon: bool = False
  unet_type: str = 'vdm', # ldm, vdm
  topk_noise_type: str = 'gamma', # gamma, gumbel
  latent_k: int = 15
  trace_matching: bool = False
...
class VDM(nn.Module):
  ...
  def __call__(self, images, labels, conditioning, step, deterministic: bool = True):
    del labels
    g_0 = jnp.squeeze(self.gamma(0.))
    g_1 = jnp.squeeze(self.gamma(1.))
    ...
    eps_hat = self.score_model(
      z_t, jnp.squeeze(g_t), conditioning[:, None], deterministic)
    ...
    if T == 0:
      ...
      s = t - (1./T)
      g_s = jnp.squeeze(self.gamma(s))
      assert g_s.shape == g_t.shape == loss_diff_mse.shape
      if self.config.reparam_type == 'input':
        loss_diff = .5 * T * jnp.exp(-g_t) * jnp.expm1(g_t - g_s) * loss_diff_mse
      elif self.config.reparam_type == 'noise':
  ...
  def sample(self, i, T, z_t, conditioning, rng):
    ...
    elif self.config.reparam_type == 'input':
      eps_hat = (
  ...
  def sde(self, xt, t):
    g_t = jnp.squeeze(self.gamma(t))
    _, g_t_grad = jax.jvp(self.gamma, (t,), (jnp.ones_like(t),))
    g_t_grad = jnp.squeeze(g_t_grad)
    g_t = g_t[:, None, None, None]
    g_t_grad = g_t_grad[:, None, None, None]
    assert g_t.ndim == g_t_grad.ndim == xt.ndim, f'{g_t.ndim} {g_t_grad.ndim} {xt.ndim}'
    drift = -0.5 * nn.sigmoid(g_t) * g_t_grad * xt
    diffusion_sqr = nn.sigmoid(g_t) * g_t_grad
    return drift, diffusion_sqr

  def apply_encoder(self, images_int):
    return jnp.zeros((images_int.shape[0], 50), dtype=jnp.float32)

  def reverse_ode(self, xt, embeddings, t, high_precision=False):
    """Create the reverse-time ODE."""
    t = t * jnp.ones((xt.shape[0],), xt.dtype)
    assert t.ndim == 1
    drift, diffusion_sqr = self.sde(xt, t)
    g_t = jnp.squeeze(self.gamma(t))
    epsilon_hat = self.score_model(
        xt,
        g_t,
        embeddings[:, :1],
        deterministic=True)
    assert drift.shape == epsilon_hat.shape, f'{drift.shape} {epsilon_hat.shape}'
    assert drift.ndim == diffusion_sqr.ndim
    g_t = g_t[:, None, None, None]
    assert g_t.shape == diffusion_sqr.shape, f'{g_t.shape} {diffusion_sqr.shape}'
    score_hat = - epsilon_hat / jnp.sqrt(nn.sigmoid(g_t))
    return drift - 0.5 * diffusion_sqr * score_hat
...
class EncDec(nn.Module):
  ...
  def logprob(self, x, z, g_0):
    # If g_0 isn't a scalar then,
    # z and g_0 should have have the same shape
...
class NoiseSchedule_Scalar(nn.Module):
  ...
  def __call__(self, t):
    if jnp.isscalar(t) or len(t.shape) == 0:
      t = t * jnp.ones((1, 1))
    return self.b[None, :] + abs(self.w[None, :]) * t[:, None]


def sigmoid(x, lamb=1):
  return 1 / (1 + jnp.exp(- lamb * x))


class NoiseSchedule_BadBDM(nn.Module):
  config: VDMConfig
  ...
  def __call__(self, t):
    config = self.config
    # values between [0, 1]
    gamma = 2 * sigmoid(t, lamb=1) - 1
    return config.gamma_min + (
      config.gamma_max - config.gamma_min) * gamma


class NoiseSchedule_GoodBDM(nn.Module):
  config: VDMConfig
...
class BlurSchedule_Scalar(nn.Module):
  config: VDMConfig
...
class BlurSchedule_FixedLinear(nn.Module):
  config: VDMConfig
...
class BlurSchedule_NNet(nn.Module):
  config: VDMConfig
  n_features: int = 1024
  nonlinear: bool = True
...
######### ResNet block #########
# use_grad_checkponting - whether or not to use gradient checkpointing
# whether to enable gradient checkpointing or not.
# Slows down training by about 20%, but significantly saves memory.
# Necessary for efficient training of models
# Alternatively, consider gradient accumulation + mixed precision
...
class ResnetBlock(nn.Module):
  ...
  @conditional_decorator(partial(nn.remat, static_argnums=(3,)), use_grad_checkpointing)
  @nn.compact
...
class AttnBlock(nn.Module):
  ...
  @conditional_decorator(nn.remat, use_grad_checkpointing)
  @nn.compact
  def __call__(self, x):
